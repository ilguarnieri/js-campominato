...
function resetGrid(){
    ...
    solution.innerHTML = '<p>Cerchi aiuto?</p><p>CLicca emoji in alto</p>';
...
function generateGrid(difficulty){
    ...
    /* AGGIUNTA NUMERI VICINO ALLE BOMBE */
    const numberAllBombs = [];
    ...
    for(let i = 0; i < bombs.length; i++){
        ...
        /* CONTROLLO SE LA BOMBA SI TROVA AL PRIMO O ULTIMO ESTREMO DELLA GRIGLIA */
        if((bombs[i] - 1) % colums_rows == 0){
            estremoSX = true;
        } else if (bombs[i] % colums_rows == 0){
            estremoDX = true;
        }
        ...
        for(let k = -1; k < 2; k++){
            ...
            for(let j = -1; j < 2; j++){
                ...
                x = bombs[i] + (k * colums_rows) + j;           

                if(x > 0 && x <= square_tot){

                    if((estremoSX && x % colums_rows != 0) || (estremoDX && (x - 1) % colums_rows != 0) || (!estremoSX && !estremoDX)){

                        numberAllBombs.push(x);
    ...
    for(let i = 1; i <= square_tot; i++){
        ...
        for(let w = 0; w < numberAllBombs.length; w++){   

            if(i == numberAllBombs[w]){

                nearBomb++;
            }
        }

        square.dataset.nearbomb = nearBomb;
    ...
    function gridCallBack(event){
        ...
        if(bombs.includes(numberSquare)){
            ...
            if(!cells.includes(numberSquare)){
                ...
                if(element.dataset.nearbomb > 0){

                    element.innerHTML = element.dataset.nearbomb;
                }
